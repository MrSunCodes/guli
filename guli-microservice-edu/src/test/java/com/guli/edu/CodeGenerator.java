...
public class CodeGenerator {
    ...
    public void genCode() {
        ...
        gc.setServiceName("%sService");	//去掉Service接口的首字母I,默认生成的第一个字母是I
        gc.setIdType(IdType.ID_WORKER_STR); //主键策略
        gc.setDateType(DateType.ONLY_DATE);//定义生成的实体类中日期类型
        gc.setSwagger2(true);//开启Swagger2模式，自动填充一些注解
        ...
        //这四个文件夹名是默认的，不写也是这四个名字，想改名可以直接修改
        ...
        strategy.setInclude(moduleName + "_\\w*");//设置要映射的表名，正则表达式\\w*表示任意个的任意字符
        strategy.setNaming(NamingStrategy.underline_to_camel);//数据库表映射到实体的命名策略，下划线转驼峰
        strategy.setTablePrefix(pc.getModuleName() + "_");//设置表前缀不生成，即取出数据库中每个表的edu_
